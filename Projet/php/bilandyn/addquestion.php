...
if (!empty(trim($_POST['cat'])) && !empty(trim($_POST['question']))) {
    ...
    $stmtExist = $conn->prepare("SELECT count(*) from bilan_questions where question = LOWER(:question) and affichage = LOWER(:affichage)");
    ...
    if ($questionCtr == 0) {
        ...
        if ($option == 'download') {
            $sql = "INSERT INTO bilan_questions(id_categorie,question,input_option,affichage,info_sup) value(:id_categorie,:question,:input_option,:affichage,:info_sup)";
