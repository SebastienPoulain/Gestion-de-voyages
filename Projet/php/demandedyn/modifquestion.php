...
if (isset($_POST["q"]) && isset($_POST['statut'])) {
    ...
    $stmtExist = $conn->prepare("SELECT count(*) from demandes_questions where question = LOWER(:question)");
    ...
    if ($questionCtr == 0) {
        $stmtExist = $conn->prepare("UPDATE demandes_questions set question = :question,info_sup = :info,actif = :statut,id_categorie = :idCat where id = :idQuestion");
