...
if(!empty($_SESSION["idprojet"])) {
  ...
  $query = "DELETE FROM projets_questions where projetID = :projetID";
  $stmt = $conn->prepare($query);
  $stmt->execute(array(':projetID' => $_SESSION["idprojet"]));

  $query = "INSERT INTO projets_questions (projetID, questionID, pourprof, pouretu, required, ordre) values (:projetID, :questionID, :pourprof, :pouretu, :required, NOW(3))";
  $stmt = $conn->prepare($query);
