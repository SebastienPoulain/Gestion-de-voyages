...
if (isset($_POST["q"]) && isset($_POST['statut'])) {
  ...
  $ancienq = $_POST["ancienq"];
    ...
    $stmtExist = $conn->prepare("SELECT count(*) from questions where question = LOWER(:question)");
  ...
  if($ancienq == $question){
    ...
    if ($questionCtr == 0) {
      ...
      $stmtExist = $conn->prepare("UPDATE questions set question = :question,info_sup = :info,actif = :statut,id_categorie = :idCat where id = :idQuestion");
